...
/*   Updated: 2022/09/21 19:12:34 by mateusteste      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_full_text(int fd, char *txt)
{
	...
	if (!buffer)
		return (NULL); 
    chars_read = 1;
	while (chars_read != 0 && !ft_strchr(txt, '\n'))
	{
...
char	*trimmed_txt(char *txt)
{
	...
	trimmed_txt = malloc((ft_strlen(txt) - i + 1) * sizeof(char));
	if (!trimmed_txt)
		return (NULL);
	while (txt[i++])
		trimmed_txt[j++] = txt[i];
